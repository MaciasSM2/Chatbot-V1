...
export const ChatHeader = ({
  ...
  const userMsgs = (messages || []).filter((m: any) => m.sender === 'user').length;
  const botMsgs = (messages || []).filter((m: any) => m.sender === 'bot').length;
  ...
  return (
    <div className={`px-4 py-3 z-10 flex items-center justify-between border-b border-border-subtle select-none transition-all duration-500 ease-in-out ${
      ...
      <div className="flex items-center gap-3">
        <div className="relative w-10 h-10 bg-avatar-bg rounded-full flex items-center justify-center font-bold text-white uppercase shrink-0 overflow-hidden">
          {companyLogoUrl ? (
