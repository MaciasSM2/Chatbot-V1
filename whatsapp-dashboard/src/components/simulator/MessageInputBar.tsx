...
export const MessageInputBar: React.FC<InputBarProps> = ({ onSendMessage }) => {
  ...
  return (
    <div className="bg-bg-header px-6 py-3 flex items-center gap-4 border-t border-border-subtle w-full">
      ...
      <form onSubmit={handleTextSubmit} className="flex-1 flex items-center gap-4">
        <input
          ...
          disabled={isUploading}
          className="flex-1 bg-bg-input border border-border-subtle rounded-xl px-4 py-3 text-xs text-text-main outline-none focus:border-[var(--theme-accent)]/30 transition-all placeholder:text-text-muted/40 font-medium"
        />
