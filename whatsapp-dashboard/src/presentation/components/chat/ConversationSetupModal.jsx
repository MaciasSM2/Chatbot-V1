...
export const ConversationSetupModal = ({ onStart, onClose }) => {
  ...
  return (
    <div className="flex items-center justify-center min-h-[500px] w-full p-4 animate-in fade-in zoom-in duration-300">
      <div className="bg-[#111b21] p-6 md:p-8 rounded-3xl border border-white/10 shadow-2xl max-w-md w-full relative overflow-hidden">
        ...
        {onClose && (
          <button 
            type="button"
            onClick={onClose}
            className="absolute top-4 right-4 p-2 text-slate-400 hover:text-slate-200 hover:bg-white/5 rounded-full transition-all cursor-pointer z-10 active:scale-95"
            title="Cerrar modal"
          >
            <X size={18} />
          </button>
        ...
        <h2 className="text-xl font-extrabold text-white mb-6 flex items-center gap-3 select-none">
        ...
        <div className="space-y-5">
          {/* Perfil del Cliente */}
          <div>
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2 select-none">Perfil del Cliente</label>
              ...
              <button 
                ...
                className={`flex-1 p-3.5 rounded-2xl border flex flex-col items-center gap-1.5 transition-all duration-300 cursor-pointer active:scale-95 ${
                  config.profile === 'NEW' 
                    ...
                    : 'bg-[#202c33] hover:bg-[#202c33]/85 border-white/5 text-slate-400'
                }`}
              >
          ...
          <div>
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2 select-none">Trato / Género</label>
            <div className="flex gap-3">
              <button 
                type="button"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2 flex items-center gap-1 select-none">
                <Calendar size={12} /> Día de Ejecución
              </label>
              <select 
                value={config.dayType}
                onChange={(e) => setConfig({...config, dayType: e.target.value})}
                className="w-full bg-[#202c33] text-slate-100 text-xs py-3 px-3.5 rounded-2xl border border-white/5 focus:border-emerald-500 focus:outline-none transition-colors cursor-pointer select-none font-bold"
              >
          ...
          <div>
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2 select-none">Categoría de Simulación</label>
            <div className="flex bg-[#202c33] p-1 rounded-2xl border border-white/5">
              <button
                type="button"
