...
export const LiveDebugSidebar = ({ currentChat, activeScenario }) => {
  const triggerStressTest = useChatStore((state) => state.triggerStressTest);

  if (!currentChat) {
    return (
      <div className="w-80 bg-[#111b21] border-l border-white/5 flex flex-col h-full items-center justify-center p-6 text-center select-none">
        <Activity size={32} className="text-slate-600 mb-3 animate-pulse" />
        <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Esperando Sesión...</h4>
  ...
  return (
    <div className="w-80 bg-[#111b21] border-l border-white/5 flex flex-col h-full overflow-hidden select-none animate-in slide-in-from-right duration-300">
      
      {/* Cabecera del Panel */}
      <div className="p-4 bg-[#202c33] flex items-center justify-between border-b border-white/5">
        <div className="flex items-center gap-2">
          <Activity size={16} className="text-emerald-500 animate-pulse" />
          <h3 className="text-xs font-black text-white uppercase tracking-wider">Motor de Simulación</h3>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-6 custom-scrollbar bg-[#111b21]">
        ...
        <div className="space-y-2.5">
          ...
          <div className="bg-[#202c33] p-3.5 rounded-2xl border border-emerald-500/20 shadow-md transition-all duration-300 hover:border-emerald-500/40">
        ...
        <div className="space-y-2.5">
          ...
          <div className="relative group">
            <div className="absolute top-3 right-3 p-1.5 bg-[#111b21] group-hover:bg-[#1f2c34] rounded-lg border border-white/5 text-[9px] font-mono text-slate-500 transition-colors uppercase">
              JSON
            </div>
            <pre className="bg-[#0b141a] p-4 rounded-2xl text-[10px] text-sky-400 font-mono overflow-x-auto border border-white/5 shadow-inner leading-relaxed custom-scrollbar max-h-64">
        ...
        <div className="space-y-3 border-t border-white/5 pt-5">
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto pr-1.5 custom-scrollbar">
            {(currentChat.metadata?.messageHistory || []).map((msg, index) => (
            ...
            {(!currentChat.metadata?.messageHistory || currentChat.metadata.messageHistory.length === 0) && (
              <p className="text-[10px] text-slate-500 italic text-center py-4 bg-[#0b141a]/40 rounded-xl border border-white/5">
        ...
        <div className="space-y-2.5">
          ...
          <div className="bg-[#0b141a] p-3.5 rounded-2xl border border-white/5 text-[9px] font-mono text-slate-400 space-y-1.5 max-h-48 overflow-y-auto custom-scrollbar">
            {(currentChat.metadata?.events || []).map((ev, i) => (
      ...
      {/* Pie del Panel */}
      <div className="p-4 border-t border-white/5 bg-[#202c33]/30 flex items-center justify-center gap-1.5">
        <Heart size={10} className="text-rose-500 fill-rose-500 animate-pulse" />
        <p className="text-[9px] text-slate-500 text-center uppercase font-black tracking-widest font-mono">
          Workbench Pro v2.0
        </p>
      </div>
    </div>
